...
const LogIn = (props) => {
    ...
    const submitForm = (e)=>{
      ...
      if(props.user.status === 1){
        console.log("Because the status changed to 1, I will load the income category...")
      }
    }
  return (
      ...
      {props.user.status === 1 ? (<Navigate replace to="/dashboard" />):(
        ...
        <div className='formContainer'>
          ...
          <div className='row'>
              ...
              <p className='text-dark'>No Account Yet? <Link to={"/signup"}  >Sign UP</Link></p>
          </div>
          <div className='row'>
              <div className='col-lg-12 form-group'>
                  <InputField value={formInput.email} name="email" 
                  processData = {processInput} type="email" label="Emailsss" 
                  id="floatingEmailss" placeholder="Emails" />
              </div>
          </div>
          <div className='row'>
              <div className='col-lg-12 form-group'>
          ...
          <div className='row'>
              <div className='col-lg-12 form-group'>
                  <button className='form-control btn btn-dark'>Login In</button>
              </div>
          </div>
          </form>
        </div>
...
const mapActionToProps = dispatch =>{
    return {
        loadExpenseCategory:()=>{dispatch(loadExpenseCategory())},
        loadIncomeCateogyr :()=>{dispatch(loadIncomeCategory())},
        signIn :(inputEmail,inputPassword)=>{dispatch(signInUser(inputEmail,inputPassword))},
